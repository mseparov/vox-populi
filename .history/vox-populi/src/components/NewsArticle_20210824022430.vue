<template>
  <div class="card">
      ...
      <form @submit.prevent="fetchSearchNews" class="d-flex" action="">
        ...
        <button @click="startSpeech" class="speech_button">Speech</button>
      </form>
...
recognition.start();
    ...
    fetchSearchNews() {
       if(this.runtimeTranscription_.includes("search")) {
          var b = this.runtimeTranscription_;
          b = b.replace("search","");
      ...
      if(this.searchword !== '')
      {
      this.apiUrl = 'https://newsapi.org/v2/everything?q=' + transcript +
                  '&pageSize=' + this.maxPerPage +
                  '&apiKey=' + this.apiKey;
      this.isBusy = true;
